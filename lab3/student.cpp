...
void pinedaTriangle(const Point &v1, const Point &v2, const Point &v3, const RGBA &color1, const RGBA &color2, bool arrow)
{
    ...
    std::vector<int> deltaE1 = { v1.x - v2.x, v1.y - v2.y };
    std::vector<int> deltaE2 = { v2.x - v3.x, v2.y - v3.y };
    std::vector<int> deltaE3 = { v3.x - v1.x, v3.y - v1.y };
    ...
    int e1 = (minX - v2.x) * deltaE1[1] - (minY - v2.y) * deltaE1[0];
    int e2 = (minX - v3.x) * deltaE2[1] - (minY - v3.y) * deltaE2[0];
    int e3 = (minX - v1.x) * deltaE3[1] - (minY - v1.y) * deltaE3[0];
    ...
    for (int Y = minY; Y < maxY; Y++) {
        ...
        e1 -= (maxX - minX) * deltaE1[1];
        e2 -= (maxX - minX) * deltaE2[1];
        e3 -= (maxX - minX) * deltaE3[1];
